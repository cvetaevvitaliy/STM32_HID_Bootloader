...
*
* Modified May 2020
*	by Vitalii Nimych <vitaliy.nimych@gmail.com>
*	add CMake and build from Clion IDE
*
*/
...
void Reset_Handler(void)
{
	 ...
	 */
	if ((magic_word == 0x424C) || READ_BIT(GPIOB->IDR, GPIO_IDR_IDR2) || (check_user_code(USER_PROGRAM) == false)) {
		if (magic_word == 0x424C) {

			/* If a magic word was stored in the
			 * battery-backed RAM registers
			 * exit from USB Serial mode and
			 * go to HID mode...
			 */
			LED2_ON;
			USB_Shutdown();
